...
from .models import Projeto, Atividade, MotivoCancelamento
...
def lista_projetos(request):
    usuario = request.user # Obtenha o usuário atual
    projetos = Projeto.objects.filter(usuario=usuario) # Filtra todos os projetos do usuário atual.
    projetos = Projeto.objects.exclude(status='cancelado')
...
def cancelar(request, projeto_id):
    projeto = get_object_or_404(Projeto, pk=projeto_id)
    if request.method == 'POST':
        motivo_cancelamento = request.POST.get('motivo_cancelamento', '')  # Obtém o motivo de cancelamento do formulário
        
        # Atualiza o status do projeto para 'cancelado'
        projeto.status = 'cancelado'
        projeto.save()
        
        # Cria um registro no modelo MotivoCancelamento
        MotivoCancelamento.objects.create(projeto=projeto, motivo=motivo_cancelamento)
        return redirect('lista_projetos')
    return render(request, 'cancelar.html')

@login_required(login_url='/login/')
def lista_cancelados(request):
    # Subquery para obter o ID do motivo mais recente de cada projeto
    subquery = MotivoCancelamento.objects.filter(projeto=OuterRef('id')).order_by('-data_cancelamento')
    
    # Query principal para selecionar os projetos cancelados e o motivo mais recente para cada um deles
    projetos_cancelados = Projeto.objects.filter(status='cancelado').annotate(
        ultimo_motivo_cancelamento=Subquery(subquery.values('motivo')[:1]),
        data_cancelamento=Subquery(subquery.values('data_cancelamento')[:1])
    )

    return render(request, 'lista_cancelados.html', {'projetos': projetos_cancelados})